  ...
  const outputSummary = useMemo(() => {
    switch (options.mode) {
      case 'extract': {
        ...
        if (count === 0) return 'No pages selected yet. Click pages in preview to pick.';
        return options.mergeExtracted
          ? `Will produce 1 merged PDF containing ${count} selected page(s).`
          : `Will produce ${count} individual separate PDF files in a ZIP archive.`;
      }
      case 'range': {
        if (!rangeValidation.valid || rangeValidation.ranges.length === 0) {
          return 'Please enter valid page slices.';
        }
        const activeCount = selectedSectionIndices.length;
        const totalSections = rangeValidation.ranges.length;
        return activeCount === 1
          ? `Will produce 1 PDF from selected section (1 of ${totalSections} sections kept).`
          : `Will produce ${activeCount} PDF file(s) in a ZIP archive (${activeCount} of ${totalSections} sections kept).`;
      }
      case 'single':
      ...
      case 'every_n': {
        ...
        return `Will split document into ${parts} separate PDF(s) of up to ${options.everyN} page(s) each in a ZIP archive.`;
      }
    }
  ...
  return (
    <Card className="flex flex-col gap-5 border border-border bg-bg-surface p-5 sm:p-6 shadow-xs">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-border pb-3">
        <div>
          <h2 className="text-base sm:text-lg font-bold text-text-main">Splitting Setup</h2>
          <p className="text-xs text-text-muted">
            Configure splitting mode and parameters. Previews and manual cut points are shown below.
          </p>
        </div>
        {result && (
      ...
      <div className="flex flex-col gap-3 pt-2">
        {options.mode === 'extract' && (
          <div className="flex flex-col gap-2 rounded border border-border bg-bg-subtle/50 p-3 text-xs">
            <span className="font-semibold text-text-main">Download Format for Extracted Pages:</span>
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              <span>Merge all selected pages into <strong>1 single continuous PDF</strong></span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer">
